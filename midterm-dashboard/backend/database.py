...
class Database:
    ...
    def get_divergence_history(
        ...
        # Two calling conventions merged: since_days=N wants a chronological
        # series (chart callers); days=N without race_key wants the biggest
        # divergences first (leaderboard callers).
        chronological = since_days is not None or bool(race_key)
        cutoff = (datetime.now(timezone.utc) - timedelta(days=since_days if since_days is not None else days)).isoformat()
        ...
        if race_key:
            ...
            order = "snapshot_time" if chronological else "max_divergence DESC"
            sql = f"""SELECT * FROM midterm_divergence_snapshots
                     WHERE snapshot_time >= ?
                     ORDER BY {order}"""
            params = (cutoff,)
