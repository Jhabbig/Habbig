...
| `econ_calendar.py` | Economic event schedule — FOMC / CPI / NFP / PCE / GDP / jobless claims and ~20 other macro releases. Pulled from FRED's `/releases/dates` endpoint, classified by name pattern into high / medium / low impact, with a 0-1 crypto-impact weight derived from historical move-on-release studies. Time-of-day hardcoded by event type (FOMC 18:00 UTC, CPI/NFP 12:30 UTC, H.4.1 20:30 UTC). High-impact events trigger a push 60 min before drop to every user with active push subscriptions. Refreshed every 6 h. Rendered in the Macro tab as "Next FOMC" + "Next CPI" countdown tiles + a 14-day list. |
| `etf_flows.py` | Spot BTC + ETH ETF tracker — institutional demand signal. Hardcoded `ETF_REGISTRY` of 20 US-listed ETFs (IBIT, FBTC, ETHA, ETHE, etc.) with current shares-outstanding metadata. Daily OHLCV from Stooq (already proven for DXY/gold). Computes per-ETF estimated AUM (close × shares), dollar volume (close × volume), 7-day AUM trend, and a bullish/neutral/bearish flow signal (AUM ↑ AND volume ≥ 1.2× 7d avg = bullish). Aggregated by asset for headline tiles. Refreshed every 4 h. Rendered as "BTC ETFs · $X AUM · ↑Y%" tile + per-ETF table in the Macro tab. |
| `cross_asset.py` | Cross-asset rotation lens — BTC vs MSTR / COIN / RIOT / MARA / CLSK / SPY / QQQ / GLD / TLT. Daily OHLCV from Stooq. Period returns (7d / 30d / YTD), rolling Pearson correlation with BTC (30d + 90d windows), 90-day beta. Series are inner-joined on equity business days so the weekend mismatch doesn't inflate short-window correlations. `normalised_series()` rebases each ticker to 100 at window start for chart overlay. Refreshed every 4 h. Rendered in the Macro tab as an inline SVG multi-line chart + comparison table with colour-coded tickers (MSTR purple, BTC orange, etc.). |
| `options.py` | Deribit options analytics — IV term structure, 25-delta skew, max pain, DVOL. Single `get_book_summary_by_currency` call returns ~5k BTC + ~2k ETH options at once; parse the instrument names (e.g. `BTC-30MAY26-100000-C`) to recover expiry + strike + side. ATM IV = avg call+put IV at strike closest to underlying. 25Δ skew = put IV minus call IV at the strikes whose deltas are closest to ±0.25 — the cleanest forward-looking risk indicator (positive = bearish hedging demand). Max pain = strike minimising total option-holder payout. DVOL = Deribit's daily volatility index. Refreshed every 30 min. Rendered in the Derivatives tab as headline tiles + ATM-IV term-structure SVG chart + full per-expiration table. |
