...
| `strategy.py` | Strategy library + backtester + marketplace. A `Strategy` is a dataclass composing DCA cadence + cycle-aware multipliers + optional harvest + optional rebalance. `backtest()` walks daily bars forward, evaluates rules per day, tracks a virtual portfolio with HIFO sell allocation, and reports equity curve + Sharpe + Sortino + max-DD + trade count. Marketplace: public visibility puts a strategy on a Sharpe-vs-drawdown leaderboard; users can fork public strategies into their private library. |
