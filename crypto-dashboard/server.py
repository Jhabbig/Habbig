...
# ── Layered .env loader ──────────────────────────────────────────────────────
# Load env from (broadest → narrowest, first definition wins):
#   ~/.gateway_env → ~/Polymarket/gateway/.env.production → ./.env.production → ./.env
# Replaces the previous "rely on systemd's EnvironmentFile" pattern that
# silently broke ODDS_API_KEY for sports-dashboard for 5 days.
try:
...
print(f"[crypto-dashboard] env files loaded: {len(_loaded_env_files)}", flush=True)
for _f in _loaded_env_files:
    print(f"  ✓ {_f}", flush=True)
if not os.getenv("GATEWAY_SSO_SECRET"):
    print("⚠ [crypto-dashboard] GATEWAY_SSO_SECRET missing — gateway-fronted requests will be rejected", flush=True)
