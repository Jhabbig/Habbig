"""Sentry initialization for the centralbank-dashboard subproduct.

Modeled on gateway/observability/sentry_setup.py and the sibling
annoyance-dashboard/observability.py. Everything is fail-soft: if
...
log = logging.getLogger("centralbank.observability")
...
def detect_release() -> str:
    ...
    if env:
        return env
    try:
        # __file__ lives at <repo>/centralbank-dashboard/observability.py
...
def init_sentry(platform: str = "centralbank") -> bool:
    """Initialize Sentry if SENTRY_DSN_CENTRALBANK (or SENTRY_DSN) is set.
    ...
    """
    dsn = (
        os.getenv("SENTRY_DSN_CENTRALBANK", "").strip()
