...
- **Two-stage prediction extraction**: precise regex first (free, fast), then Claude (LLM fallback) for natural-language predictions the regex misses. Results are cached by content hash so repeat posts cost nothing. Defaults to `claude-opus-4-8` with adaptive thinking; swap to `claude-haiku-4-5` via `LLM_EXTRACTOR_MODEL` for lower cost.
...
- **Prediction Engine (Stage 3)** — `/api/v1/engine/predict` fuses credibility scores + LLM-extracted metrics into a calibrated P(YES) with confidence, at concurrent-user scale: content-hash dedup cache (Redis-ready), per-job-class model tiers (hot-reloaded config, no redeploy), Batch-API path for offline jobs (-50%), full audit trail for reproducibility, replay harness for accuracy/calibration grading. See `app/engine/README.md`.
