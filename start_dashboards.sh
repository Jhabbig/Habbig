...
#   8053 — Whale Tracker Dashboard   (whale-tracker-dashboard/server.py)
#
...
ALL_PORTS="7000 8000 8050 8051 8052 5050 8888 7050 7060 8053"
...
start_all() {
    ...
    echo -e "${GREEN}[1/10]${NC} Starting Crypto Dashboard on port 8000..."
    ...
    echo -e "${GREEN}[2/10]${NC} Starting Stock Dashboard on port 8050..."
    ...
    echo -e "${GREEN}[3/10]${NC} Starting Midterm Dashboard on port 8051..."
    ...
    echo -e "${GREEN}[4/10]${NC} Starting Top Traders Dashboard on port 8052..."
    ...
    echo -e "${GREEN}[5/10]${NC} Starting Weather Dashboard on port 5050..."
    ...
    echo -e "${GREEN}[6/10]${NC} Starting Sports Dashboard on port 8888..."
    ...
    echo -e "${GREEN}[7/10]${NC} Starting World State Dashboard on port 7050..."
    ...
    # 9. Whale Tracker Dashboard (port 8053)
    echo -e "${GREEN}[9/10]${NC} Starting Whale Tracker Dashboard on port 8053..."
    cd "$SCRIPT_DIR/whale-tracker-dashboard"
    python3 server.py > /tmp/dashboard_whales.log 2>&1 &
    echo $! > /tmp/dashboard_whales.pid
    echo "       PID: $(cat /tmp/dashboard_whales.pid)"
    cd "$SCRIPT_DIR"
    ...
    echo -e "  ${GREEN}Whale Tracker Dashboard:${NC} http://localhost:8053"
    echo ""
...
status() {
    ...
    echo -e "  Port 8053 (Whales):   $(lsof -ti :8053 >/dev/null 2>&1 && echo -e "${GREEN}RUNNING${NC}" || echo -e "${RED}STOPPED${NC}")"
    echo ""
}
