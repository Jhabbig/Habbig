...
#   7053 — Voter Mood Dashboard      (voter-dashboard/server.py)
#
...
ALL_PORTS="7000 8000 8050 8051 8052 5050 8888 7050 7060 7053"
...
start_all() {
    ...
    # 9. Voter Mood Dashboard (port 7053)
    echo -e "${GREEN}[9/10]${NC} Starting Voter Mood Dashboard on port 7053..."
    PORT=7053 python3 "$SCRIPT_DIR/voter-dashboard/server.py" > /tmp/dashboard_voter.log 2>&1 &
    echo $! > /tmp/dashboard_voter.pid
    echo "       PID: $(cat /tmp/dashboard_voter.pid)"
    ...
    echo -e "  ${GREEN}Voter Mood Dashboard:${NC}  http://localhost:7053"
    echo ""
...
status() {
    ...
    echo -e "  Port 7053 (Voter):    $(lsof -ti :7053 >/dev/null 2>&1 && echo -e "${GREEN}RUNNING${NC}" || echo -e "${RED}STOPPED${NC}")"
    echo ""
}
