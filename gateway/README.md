...
| File | Purpose |
|---|---|
| `config.json` | **Edit this** to change subdomain names, prices, target ports, or accent colors per dashboard. The internal `key` for each dashboard must NOT change. Per-dashboard flags: `"hidden": true` delists a dashboard from every storefront surface (landing, hub, tour, billing, settings) while proxying and existing subscriptions keep working; `"access_alias": "<key>"` makes a subscription to that other key unlock this dashboard too (how stocks+crypto sell as one Market Edge product); `"merged_into": "<key>"` retires a dashboard whose subdomain then 301-redirects to the absorbing one (pair with top-level `"legacy_grants"` so old subscribers keep access — how disasters/climate fold into weather); `"parked": true` marks a hidden dashboard whose service is intentionally stopped — its subdomain serves a parked notice, health probes skip it, and `/admin/fleet` tallies the visits. |
