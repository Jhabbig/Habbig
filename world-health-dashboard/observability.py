"""Sentry initialization for the world-health-dashboard subproduct.
...
log = logging.getLogger("world_health.observability")
...
def detect_release() -> str:
    ...
    if env:
        return env
    try:
        # __file__ lives at <repo>/world-health-dashboard/observability.py
...
def init_sentry(platform: str = "world-health") -> bool:
    """Initialize Sentry if SENTRY_DSN_HEALTH (or SENTRY_DSN) is set.
    ...
    """
    dsn = (
        os.getenv("SENTRY_DSN_HEALTH", "").strip()
