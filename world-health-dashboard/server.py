...
_AUTH_BYPASS_EXACT = {"/health", "/healthz", "/api/health", "/favicon.ico", "/manifest.webmanifest"}
...
async def _sentry_test(request: Request) -> dict:
    ...
    raise RuntimeError("Sentry test event — this is intentional (world-health-dashboard)")
...
def manifest() -> JSONResponse:
    return JSONResponse(
        {
            "name": "narve.ai — World Health",
            "short_name": "World Health",
            "start_url": "/",
            "display": "standalone",
