...
class RiskManager:
    ...
    def calculate_kelly(self, signal: Signal) -> float:
        ...
        if signal.action == "BUY_YES":
            p = signal.model_prob
            yes_price = getattr(signal, "exec_price_yes", None)
            ...
            no_price = getattr(signal, "exec_price_no", None)
            if no_price is None:
    ...
    def size_position(self, signal: Signal) -> PositionSize:
        ...
        if self.daily_pnl <= -self.config.DAILY_LOSS_LIMIT:
            return PositionSize(0.0, 0.0, 0.0, f"Daily loss limit hit (PnL: ${self.daily_pnl:.2f})", False)

        existing = self.open_positions.get(signal.market.condition_id, 0.0)
        if existing > 0:
            return PositionSize(0.0, 0.0, 0.0, f"Already have ${existing:.2f} position in this market", False)
        ...
        if amount < 1.0:
            return PositionSize(0.0, kelly, adjusted, f"Position size too small (${amount:.2f})", False)
