...
def test_sources_shape_order_and_movement(client):
    ...
    assert race["kind"] == "model"
    assert by_id["sample:capitol_staffer"]["kind"] == "user"
...
def test_questions_shape_and_edge_sort(client):
    ...
    top = rows[0]
    # blend: (0.62*0.8125 + 0.66*0.5)/(0.8125+0.5) = 0.63524; edge vs 0.55
    assert top["id"] == "midterm-house-gop-2026" and top["edge"] == 0.0852
    assert top["n_sources"] == 2 and top["is_sample"] is True
...
def test_question_detail_shape(client):
    ...
    assert [(r["source_id"], r["credibility"], r["p"])
            for r in body["per_source"]] == [
        ("sample:race_model", 0.8125, 0.62),
        ("sample:capitol_staffer", 0.5, 0.66)]
...
def test_resolve_endpoint_and_409(client):
    ...
    moves = {m["source_id"]: m for m in body["moves"]}
    assert moves["sample:race_model"] == {
        "source_id": "sample:race_model", "old_cred": 0.8125,
        "new_cred": round(14 / 17, 4), "hit": True}
    assert moves["sample:capitol_staffer"] == {
        "source_id": "sample:capitol_staffer", "old_cred": 0.5,
