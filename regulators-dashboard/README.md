...
| **v2.6** | ✓ done | **UX polish: item-detail modal + keyboard nav + search highlight.** Click any row's `⋯` button (or press `Enter` on a keyboard-selected row) to open a modal showing the full classification breakdown — matched phrases per category, severity native/USD/context-word, topic matches, prediction-market joins with shared tokens. Keyboard shortcuts: `/` focuses search, `j`/`k` move row selection, `Enter` opens details, `Esc` closes. Search results highlight the matched substring inline with `<mark>`. Header carries a small `/` `j/k` `Enter` `Esc` hint so the shortcuts are discoverable. |
